...
function appendRepairInstruction(body, kind) {
  ...
  const instruction = REPAIR_INSTRUCTIONS[kind];
  if (!instruction) return repaired;
...
export class KiroExecutor extends BaseExecutor {
  ...
  async runIntegrityRecovery(rawResponse, args, options) {
    ...
    const repairKind = first.kind === "invalid_tool" ? "tool" : null;
    const repairBody = repairKind
      ? appendRepairInstruction(args.body, repairKind)
